...
(function ($) {
    $.fn.tableExport = function (options) {
        ...
        if (defaults.type === "csv" || defaults.type === "tsv" || defaults.type === "txt") {
            ...
            let save_data;
            if (defaults.jsonScope === "head") save_data = JSON.stringify(jsonHeaderArray);
            else if (defaults.jsonScope === "data") save_data = JSON.stringify(jsonArray);
            ...
            $(el).filter(function () {
                ...
                const $table = $(this);
                let ssName = "";
                if (typeof defaults.mso.worksheetName === "string" && defaults.mso.worksheetName.length) ssName = defaults.mso.worksheetName + " " + (docNames.length + 1);
                else if (typeof defaults.mso.worksheetName[docNames.length] !== "undefined") ssName = defaults.mso.worksheetName[docNames.length];
            ...
            for (let n = 0, c = docNames.length; n < c; n++) {
                ...
                if (count[item] > 1) docNames[n] = docNames[n].substring(0, 29) + "-" + count[item];
                else firstOccurrences[item] = n
            }
            ...
            $(el).filter(function () {
                ...
                let sheetName = "";
                if (typeof defaults.mso.worksheetName === "string" && defaults.mso.worksheetName.length) sheetName = defaults.mso.worksheetName + " " + (sheetNames.length + 1);
                else if (typeof defaults.mso.worksheetName[sheetNames.length] !== "undefined") sheetName = defaults.mso.worksheetName[sheetNames.length];
                        ...
                        if (cell !== null) {
                            ...
                            if (defaults.mso.styles.length) {
                                ...
                                for (let cssStyle in defaults.mso.styles) {
                                    ...
                                    if (thCss === "") thCss = rowStyles[defaults.mso.styles[cssStyle]];
                        ...
                        if (cell !== null) {
                            ...
                            if (defaults.mso.styles.length) {
                                ...
                                for (let cssStyle in defaults.mso.styles) {
                                    ...
                                    if (tdCss === "") tdCss = rowStyles[defaults.mso.styles[cssStyle]];
                ...
                loadImages(teOptions, function () {
                    $(el).filter(function () {
                        ...
                        if (typeof teOptions.onTable === "function")
                            if (teOptions.onTable($(this), defaults) === false) return true;
                                ...
                                if (typeof teOptions.columns[data.column.dataKey] !== "undefined") {
                                    ...
                                    if (typeof col.rect !== "undefined") {
                                        ...
                                        if (typeof teOptions.heightRatio === "undefined" || teOptions.heightRatio === 0) {
                                            if (data.row.raw[data.column.dataKey].rowspan) rh = data.row.raw[data.column.dataKey].rect.height / data.row.raw[data.column.dataKey].rowspan;
                        ...
                        if (typeof atOptions.drawHeaderCell !== "function") {
                            atOptions.drawHeaderCell = function (cell, data) {
                                ...
                                if ((colopt.style.hasOwnProperty("hidden") !== true || colopt.style.hidden !== true) && colopt.rowIndex >= 0) return prepareAutoTableText(cell, data, colopt);
                                else return false
                            }
                        }
            ...
            if (checkCellVisibility) {
                hiddenEls = $hiddenTableElements.filter(function () {
                    ...
                    if (this.nodeType === $element[0].nodeType) {
                        if (typeof this.rowIndex !== "undefined" && this.rowIndex === $element[0].rowIndex) found = true;
                        else if (typeof this.cellIndex !== "undefined" && this.cellIndex === $element[0].cellIndex && typeof this.parentNode.rowIndex !== "undefined" && typeof $element[0].parentNode.rowIndex !== "undefined" && this.parentNode.rowIndex === $element[0].parentNode.rowIndex) found = true
                    }
                    return found
                })
            }
        ...
        function jsPdfOutput(doc, hasimages) {
            ...
            try {
                ...
                if (typeof defaults.onAfterSaveToFile === "function") defaults.onAfterSaveToFile(blob, fileName)
            } catch (e) {
        ...
        function prepareAutoTableText(cell, data, cellopt) {
            ...
            if (cs >= 0) {
                ...
                if (cs > 1) {
                    if (cell.styles.halign === "right") textPosX = cell.textPos.x + cellWidth - cell.width;
                    else if (cell.styles.halign === "center") textPosX = cell.textPos.x + (cellWidth - cell.width) / 2
                }
        ...
        function parseDateUTC(s) {
            ...
            if (o.getFullYear() === y && o.getMonth() === m - 1 && o.getDate() === d) return new Date(Date.UTC(y, m - 1, d, 0, 0, 0));
            else return false
        }
                    ...
                    if (typeof defaults.onCellHtmlData === "function") {
                        ...
                        $.each(html, function () {
                            if ($(this).is("input")) {
                                ...
                                if (typeof $(this).html() === "undefined") htmlData += $(this).text();
                                else if (jQuery().bootstrapTable === undefined || $(this).hasClass("fht-cell") === false && $(this).hasClass("filterControl") === false && $cell.parents(".detail-view").length === 0) htmlData += $(this).html();
                                if ($(this).is("a")) {
        ...
        function xlsxTableToSheet(table) {
            ...
            const sheetRows = Math.min(1e7, rows.length);
            const range = {
                s: {
                    r: 0,
                    c: 0
            ...
            for (; _R < rows.length && R < sheetRows; ++_R) {
                ...
                for (_C = C = 0; _C < elts.length; ++_C) {
                    ...
                    if (cellFormat !== "") {
                        ...
                        if (ssfId === 0 && typeof defaults.mso.xslx.formatId.numbers === "function") ssfId = defaults.mso.xslx.formatId.numbers($(elt), _R, _C + CSOffset);
                        if (ssfId === 0 && typeof defaults.mso.xslx.formatId.date === "function") ssfId = defaults.mso.xslx.formatId.date($(elt), _R, _C + CSOffset);
                        if (ssfId === 49 || ssfId === "@") _t = "s";
                        else if (cellInfo.type === "number" || ssfId > 0 && ssfId < 14 || ssfId > 36 && ssfId < 41 || ssfId === 48) _t = "n";
                        else if (cellInfo.type === "date" || ssfId > 13 && ssfId < 37 || ssfId > 44 && ssfId < 48 || ssfId === 56) _t = "d"
                    } else _t = "s";
                    if (v != null) {
                        let vd;
                        if (v.length === 0) {
        ...
        function jsPdfCalculateWidths() {
            ...
            all.forEach(function (row, i) {
                ...
                row.height = row.heightStyle + lineBreakCount * row.styles.fontSize * FONT_ROW_RATIO + (2 - FONT_ROW_RATIO) / 2 * row.styles.fontSize;
